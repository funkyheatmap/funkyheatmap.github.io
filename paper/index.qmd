...
Different **geometries** are supported depending on the data type of each of the columns ([@tbl-geoms]). For instance, numerical data can best be represented by a funkyrect or bar geom, measurement data by labelled rectangle geom, categorical data by a text or image geom, and proportional data a pie-chart geom.

```{r echo=FALSE}
#| label: tbl-geoms
#| tbl-cap: Recommended geoms
df <- tibble::tribble(
...
In order to provide a user-friendly experience for Python-based workflows, we implemented the Python package `funkyheatmappy` which relies on the ubiquitous matplotlib library [@matplotlib2dgraphics_hunter2007]. To guarantee feature parity between the different `{funkyheatmap}` implementations, test data is shared between the implementation tests, ensuring that the resulting plots look identical.

To allow for interactive exploration of the results, a JavaScript port called `funkyheatmapjs` is being developed. This version provides some extra functionality, allowing you to sort the rows by clicking on a column and to show a label on hover.
